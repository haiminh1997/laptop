...
<?php 
    ...
    **/
    ...
    /**
    * tao slug,chuyển từ tên sang dạng slug k có dấu cách
    **/
    ...
    function to_slug($str) {
     ...
     if( ! function_exists('xss_clean') ) { // xlear các ký tự đặc biệt của file input đầu vào
    ...
    function  getInput($string) //lấy dữ liệu (sử dụng với edit.php)
    {
    ...
    function  postInput($string) // thêm dữ liệu (sử dụng với add.php)
    { // nếu tồn tại post thì lấy gtri,k tồn tại thì trả về rỗng
    ...
    function base_url()
    {
        ...
        return $url  = "http://localhost/laptop/FshareShop/";
    }
    ...
    /**
     *  redirect về các trang trong phần admin
     */
     ...
     */
    if ( ! function_exists('redirect')) //kiểm tra nếu chưa tồn tại hàm này thì sẽ tạo hàm
    {
    ...
    function formatPrice($number){ // tạo dấu chấm cho giá sp 9.000.000
         $number = intval($number); // ép kiểu
         return $number = number_format($number,0,',','.')."₫";
    }


    function formatpricesale($number,$sale){ // tính giá sale của sp
