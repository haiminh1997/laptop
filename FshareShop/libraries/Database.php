...
<?php 
    ...
    */
    class Database
    {
        ...
        public function __construct() //khi khởi tạo lớp dtb,mặc định sẽ gọi function construct
        {
            $this->link = mysqli_connect("localhost","root","","fshareshop") or die ();
            mysqli_set_charset($this->link,"utf8");// chuyển về tiếng việt có dấu   
        }
        ...
        // Thêm dữ liệu vào 1 bảng theo từng bảng là "table", lấy dữ liệu theo kiểu mảng "array $data"
        // Ví dụ :
        //  bảng user (id,name,images,phone,email)
        //  $data= 
        //  [
        //     'id' là giá trị tự động tăng
        //      'name' => giá trị name,
        //     'image' => ...
        //     'phone' => ...
        //     'email' => ...
        ...
        public function update($table, array $data, array $conditions) // 3 giá trị tên bảng,mảng data đã khai báo trên, condition là điều kiện
        {
            $sql = "UPDATE {$table}";
         ...
         */
        public function delete ($table ,  $id ) // truyền vào 1 bảng và id
        {
            $sql = "DELETE FROM {$table} WHERE id = $id ";

            mysqli_query($this->link,$sql) or die (" Lỗi Truy Vấn delete   --- " .mysqli_error($this->link));
            return mysqli_affected_rows($this->link); // trả về =0 thì thành công,>0 thì thất bại
        }
        ...
        {//lấy tất cả các sản phẩm có id bằng id mình truyền vào
            $sql = "SELECT * FROM {$table} WHERE id = $id ";
            $result = mysqli_query($this->link,$sql) or die("Lỗi  truy vấn fetchID " .mysqli_error($this->link));
            return mysqli_fetch_assoc($result); // trả về dữ liệu với id k trung nhau
        }

        public function fetchOne($table , $query) 
        {//lấy ra 1 bản ghi,tham số vào alf 1 bảng và tham số query
        ...
        public function deletesql ($table ,  $sql )
        {
         ...
         public function fetchAll($table) //truyền vào 1 tên bảng, k có điều kiện gì và lấy hết dữ liệu của bảng đó
        {
            $sql = "SELECT * FROM {$table} WHERE 1" ;
            $result = mysqli_query($this->link,$sql) or die("Lỗi Truy Vấn fetchAll " .mysqli_error($this->link));
            $data = []; // tạo 1 mảng data rỗng, sau đó kết quả sẽ đc trả vào mảng đó
            if( $result)
            {
                while ($num = mysqli_fetch_assoc($result))
                {
    ...
    // Giúp phân trang,tham số truyền vào là bảng,câu lệnh truy vấn sql,số bản ghi muốn hiển thị trên 1 trang,số trang,số cột,trạng thái
        public  function fetchJones($table,$sql,$total = 1,$page,$row ,$pagi = true )
        { // $row số bản ghi trên 1 trang,$total tổng số bản ghi
            
            $data = [];

            if ($pagi == true ) // nếu page=true thì phân trang,k thì thôi
            {
